...
# Set kitaran aktif lalai jika tiada
if "kitaran_aktif" not in st.session_state:
    st.session_state.kitaran_aktif = "Kitaran Semasa"
...
# ================== LOGIK KIRAAN HAID & CARIAN BULANAN ==================
def kira_rumusan_haid_aktif():
    total_haid = {}
    if st.session_state.data:
        for date_str, info in st.session_state.data.items():
            if isinstance(info, dict):
                # Hanya kira data yang belum di-reset (flag reset_haid != True)
...
# ================== POPUP BORANG LAPORAN HARIAN (AMAN DARI BUG POPUP KUNCI) ==================
...
def papar_popup_laporan(date_key):
    ...
    # Sediakan struktur data yang selamat di dalam session_state
    ...
    if st.button("➕ Tambah Kes Salah Laku"):
        if kesalahan_text.strip() != "":
            # Kemas kini data terus ke dalam salinan session_state secara selamat
...
# Memastikan dialog hanya dipanggil jika active_date benar-benar wujud
...
with col2:
    ...
    if murid_amaran:
        ...
        for m_nama, m_info in murid_amaran.items():
            with st.expander(f"🔴 {m_nama} — Bersamaan ({m_info['hari']} Hari)"):
    ...
    if st.button("🔄 Tarik Paksa Data GitHub", use_container_width=True):
        st.session_state.data = ambil_data_dari_github()
        st.session_state.active_date = None  # Clear sebarang tarikh sangkut semasa force pull
        st.rerun()