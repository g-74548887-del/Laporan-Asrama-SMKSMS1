...
# ================== FUNGSI BACA GITHUB (ANTI-CACHE & PADU) ==================
def ambil_data_dari_github():
    try:
        # Menambah query parameter 'ref' dengan timestamp unik untuk MEMAKSA github beri data paling baru (Bypass Cache)
        timestamp = str(datetime.datetime.now().timestamp())
        file_content = repo.get_contents(FILE_PATH, ref=timestamp)
        ...
        # Jika ada ralat, paparkan di skrin supaya kita tahu kenapa dia gagal baca
        st.error(f"⚠️ Ralat Membaca GitHub: {e}")
        return {}

# PENGUNCI MEMORI SESI STREAMLIT
# Sentiasa paksa ambil data dari GitHub jika session state kosong semasa refresh
...
# ================== FUNGSI SIMPAN GITHUB NEW (ANTI-OVERWRITE) ==================
def hantar_data_ke_github(data_baru, msg):
    try:
        timestamp = str(datetime.datetime.now().timestamp())
        file = repo.get_contents(FILE_PATH, ref=timestamp)
        ...
        # Kemaskini memori lokal Streamlit dengan data yang baru dihantar
...
# ================== JANA SEMUA TARIKH (MULA BULAN 6) ==================
def gen_dates():
...
def papar_popup_laporan(date_key):
    ...
    if st.button("💾 Simpan Semua Rekod Laporan", use_container_width=True):
        ...
        # Ambil pangkalan induk terkini tanpa cache dari GitHub
...
with col2:
    ...
    if st.button("🔄 Tarik Paksa Data GitHub", use_container_width=True):
        # Butang tarik paksa juga diletakkan anti-cache
        st.session_state.data = ambil_data_dari_github()
        st.rerun()