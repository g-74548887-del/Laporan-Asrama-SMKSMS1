...
with col1:
    ...
    st.subheader("Keceriaan")
    for t in ["Dorm","Bilik","Surau","Tandas"]:
        key = f"CERIA_{t}"
...
if st.session_state.popup_type == "tarikh":
    ...
    key = st.session_state.popup_key
    ex = st.session_state.data.get(key, {})

    st.markdown(f"## 📋 Laporan {key}")
    ...
    masa = st.text_input("Masa", value=ex.get("masa_rondaan",""))
...
if st.session_state.popup_type == "rohani":
    ...
    st.markdown(f"## 📖 Kerohanian {key}")

    nama = st.text_input("Nama", value=ex.get("nama",""))
