...
def save_data(data):
    ...
    try:
        file = repo.get_contents(FILE_PATH)
        repo.update_file(FILE_PATH, "update laporan", json.dumps(data, indent=4), file.sha)
    except:
        repo.create_file(FILE_PATH, "create laporan", json.dumps(data, indent=4))
...
def save_denda(data):
    ...
    try:
        file = repo.get_contents(DENDA_FILE)
        repo.update_file(DENDA_FILE, "update denda", json.dumps(data, indent=4), file.sha)
    except:
        repo.create_file(DENDA_FILE, "create denda", json.dumps(data, indent=4))
...
# 🔥 AMBIL NAMA UPPERCASE DI DEPAN AYAT SAHAJA
def extract_nama_upper(text):
    if not text:
        return []
...
if st.session_state.selected_date:
    ...
    def laporan_popup():
        ...
        with st.form("laporan_form"):
            ...
            nama_warden = st.text_input("Nama Warden Bertugas", value=existing.get("nama_warden", ""))
            warden_oncall = st.text_input("Nama Warden Oncall", value=existing.get("warden_oncall", ""))
            masa_rondaan = st.text_input("Masa Rondaan", value=existing.get("masa_rondaan", ""))

            bil_murid = st.number_input("Bilangan Murid", min_value=0, value=existing.get("bil_murid", 0))
            murid_tiada = st.text_input("Murid Tiada & Sebab", value=existing.get("murid_tiada", ""))

            st.markdown("### Kes Salah Laku")
            st.info("⚠️ Contoh: SITI NURHALIZA tidak solat subuh berjemaah\n(Hanya NAMA HURUF BESAR akan masuk Murid Denda)")
            ...
            catatan_program = st.text_area("Catatan / Program", value=existing.get("catatan_program", ""))
            ...
            if submitted:
                ...
                # 🔥 AMBIL NAMA SAHAJA
                all_kes = kes1 + "\n" + kes2 + "\n" + kes3
                nama_list = extract_nama_upper(all_kes)

                for nama in nama_list:
                    if nama not in st.session_state.murid_denda:
                        st.session_state.murid_denda.append(nama)
