...
def load_json(path, default):
    try:
        ...
        # Jika gagal load dari GitHub, beri amaran dan jangan pulangkan dict kosong melainkan fail asal memang kosong
        st.error(f"⚠️ Amaran: Gagal menghubungi GitHub untuk memuatkan data lama ({e}). Sila refresh halaman atau klik 'Ambil Data Terkini'.")
        return None
...
if "data" not in st.session_state or st.session_state.data is None:
    ...
    if loaded_data is not None:
        ...
        st.session_state.data = {} # Hanya fallback terakhir jika ralat kritikal
...
def papar_popup_laporan(date_key):
    ...
    # Ambil data terkini dari session state
    ...
    if st.button("💾 Simpan Semua Rekod Laporan", use_container_width=True):
        ...
        # Ambil semula data terkini dari GitHub SEBELUM save, bagi mengelakkan overwriting data orang lain
        ...
        if fresh_db is not None:
            ...
            st.error("Gagal menyimpan kerana sistem gagal mengesahkan pangkalan data induk di GitHub. Sila cuba sebentar lagi.")
