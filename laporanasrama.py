...
with col2:
    ...
    for m, v in sorted(total.items(), key=lambda x: x[1], reverse=True):
        ...
        with colB:
            if st.button("🧹", key=f"reset_total_{m}"):
                for t in st.session_state.data:
                    st.session_state.data[t].get("haid_data", {})[m] = 0
                save_json(FILE_PATH, st.session_state.data, "reset haid")
                st.rerun()
