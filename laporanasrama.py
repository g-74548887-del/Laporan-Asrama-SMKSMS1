...
# ================== CACHE DATA UNTUK KUNCI DATA DARIPADA HILANG ==================
@st.cache_data(ttl=60)  # Data disimpan selamat dalam cache selama 60 saat walaupun di-refresh
def load_json_from_github():
    try:
        file_content = repo.get_contents(FILE_PATH)
        if file_content.size <= 2: # Jika fail kosong atau sekadar {}
            return {}
...
# ================== SAVE JSON & CLEAR CACHE ==================
def save_json_global(updated_db, msg):
    try:
        file = repo.get_contents(FILE_PATH)
        content = json.dumps(updated_db, indent=4)
        repo.update_file(FILE_PATH, msg, content, file.sha)
        st.cache_data.clear()  # Kosongkan cache supaya data baru dipaparkan terus
        st.session_state.data = updated_db
        st.success("💾 Berjaya disimpan ke GitHub secara kekal!")
    except Exception as e:
        st.error(f"❌ Gagal menyimpan ke GitHub: {e}")

# INITIALIZATION SESI STREAMLIT
if "data" not in st.session_state or not st.session_state.data:
    st.session_state.data = load_json_from_github()
...
def papar_popup_laporan(date_key):
    ...
    if st.button("💾 Simpan Semua Rekod Laporan", use_container_width=True):
        ...
        # Tarik pangkalan data induk terkini sebelum simpan
        st.cache_data.clear()
        fresh_db = load_json_from_github()
        fresh_db[date_key] = existing
        save_json_global(fresh_db, f"Simpan laporan lengkap bertarikh {date_key}")
        st.session_state.active_date = None
        st.rerun()
