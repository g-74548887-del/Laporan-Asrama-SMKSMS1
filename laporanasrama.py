...
def kira_rumusan_haid_keseluruhan():
    ...
    for date_str, info in st.session_state.data.items():
        if isinstance(info, dict):
            # Menyokong kedua-dua key 'haid_hari_ini' dan 'haid_data' dari JSON
...
with col1:
    st.subheader("📅 Senarai Tarikh Laporan (2026-06-08 ➡️ 2026-12-05)")
    with st.container(height=650):
        for d in all_dates:
            ...
            # DIKEMASKINI: Memaparkan tarikh sahaja terus tanpa perkataan lain
...
def papar_popup_laporan(date_key):
    ...
    ex_haid_popup = []
    # Membaca data haid sedia ada dari kedua-dua key alternatif
