...
with col2:
    ...
    # Dapatkan susunan hari dalam kalendar menggunakan perpustakaan calendar python
    cal = calendar.Calendar(firstweekday=6) # 6 bermaksud Ahad bermula dahulu
    weeks = cal.monthdayscalendar(pilihan_tahun, bulan_angka)
    
    # Rekabentuk gaya CSS untuk grid jadual kalendar
    html_content = """
    <style>
    ...
    for week in weeks:
        ...
        for day in week:
            if day == 0:
                ...
                # Semak senarai pelajar haid pada tarikh ini
                    ...
                    if isinstance(info_tarikh, dict):
                        ...
                        if isinstance(haid_list, dict):
                            for murid_nama, n_hari in haid_list.items():
                                if int(n_hari) > 0:
                                    # Ambil 2 nama pertama untuk ringkasan di kotak kalendar supaya muat
    ...
    # Papar HTML Kalendar secara selamat di Streamlit
