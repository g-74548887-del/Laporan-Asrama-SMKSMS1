...
# ================== SIDEBAR MENU ==================
menu = st.sidebar.radio("Menu", ["Laporan Harian", "Murid Denda"])
        ...
        def laporan_popup():
            ...
            with st.form("laporan_form"):
                ...
                if submitted:
                    ...
                    st.success("✅ Laporan disimpan (termasuk murid denda)")
                    st.session_state.selected_date = None
                    st.rerun()
...
if menu == "Murid Denda":
    ...
    st.header("📌 Senarai Murid Denda")

    if len(st.session_state.murid_denda) == 0:
        st.info("Tiada murid dalam senarai denda")
    else:
        for i, murid in enumerate(st.session_state.murid_denda):

            col1, col2, col3 = st.columns([4,2,1])

            col1.write(f"👤 {murid['nama']}")
            col2.write(f"📅 {murid['tarikh']}")
