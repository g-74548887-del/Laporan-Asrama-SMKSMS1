...
# ================== FUNGSI BACA GITHUB (VERSI BETUL & STABIL) ==================
def ambil_data_dari_github():
    try:
        # Dibuang parameter ref=timestamp yang menyebabkan ralat 404 tadi
...
# ================== FUNGSI SIMPAN GITHUB (VERSI CUN) ==================
def hantar_data_ke_github(data_baru, msg):
    try:
        # Ambil fail asal untuk dapatkan nilai SHA terkini (Wajib untuk update fail di GitHub)
        ...
        # Selesai simpan ke GitHub, paksa memori lokal Streamlit dikemas kini terus
...
def papar_popup_laporan(date_key):
    ...
    if st.button("💾 Simpan Semua Rekod Laporan", use_container_width=True):
        ...
        # Ambil pangkalan induk terkini terus dari GitHub sebelum gabung data (Anti-Overwrite)
        pangkalan_induk = ambil_data_dari_github()
        pangkalan_induk[date_key] = existing
        
        # Hantar data baru yang lengkap ke GitHub
