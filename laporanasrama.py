...
# ================== LOGIK BAHARU: AMBIL DATA TERAKHIR DI-KEYIN SAHAJA ==================
def kira_rumusan_haid_keseluruhan():
    total_haid = {m: 0 for m in MURID}
    
    # Susun tarikh dari lama ke baru supaya tarikh paling akhir akan overwrite nilai lama
    for t in sorted(st.session_state.data.keys()):
        info = st.session_state.data[t]
        if isinstance(info, dict):
            ...
            if isinstance(haid_tarikh, dict):
                for m, hari in haid_tarikh.items():
                    if m in total_haid:
                        try:
                            # Mengambil nilai terakhir yang di-keyin secara langsung (bukan tambah)
        ...
        for m in MURID:
            ...
            with col_nama:
                if hari_tarikh_ini > 0:
                    st.markdown(f"🔴 **{m} ({hari_tarikh_ini} Hari)**")
                else:
                    st.write(f"⚪ {m} ({total_terkini} Hari)")
