...
# ================== FUNGSI SIMPAN GITHUB NEW (ANTI-PADAM & ANTI-CORRUPT) ==================
def hantar_data_ke_github(data_baru, msg):
    try:
        file = repo.get_contents(FILE_PATH)
        # Tukar data json kepada string dengan format susunan yang cantik
        json_string = json.dumps(data_baru, indent=4)
        # KAEDAH BARU: Tukar kepada bytes (utf-8) sebelum hantar supaya fail tidak rosak/kosong di GitHub
...
# ================== JANA SEMUA TARIKH (BERMULA BULAN 6) ==================
def gen_dates():
    # Diubah untuk mula dari 1 Jun 2026 (Bulan 4 telah dibuang)
...
def papar_popup_laporan(date_key):
    ...
    if st.button("💾 Simpan Semua Rekod Laporan", use_container_width=True):
        ...
        # === KAEDAH ANTI-PADAM DATA KAWAN/LAMA ===
        # Tarik data paling terkini dari GitHub dulu supaya data yang dimasukkan oleh warden lain tidak hilang
        pangkalan_induk = ambil_data_dari_github()
        
        # Gabung atau kemas kini tarikh aktif sahaja tanpa menyentuh tarikh-tarikh lain
        pangkalan_induk[date_key] = existing
        
        # Simpan semula keseluruhan pangkalan data yang lengkap ke GitHub
        hantar_data_ke_github(pangkalan_induk, f"Kemaskini laporan tarikh {date_key}")
        
        st.session_state.active_date = None
        st.rerun()

# PANGGIL POPUP JIKA TARIKH AKTIF DIPILIH
