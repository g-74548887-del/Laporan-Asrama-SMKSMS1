...
def analisis_haid_bulanan(bulan_pilihan):
    # Mengumpul maklumat mengikut format: { nama: {"hari": X, "tarikh": [t1, t2...]} }
    laporan_bulan = {}
    if st.session_state.data:
        for date_str, info in st.session_state.data.items():
            try:
                # Ambil integer bulan daripada string tarikh (YYYY-MM-DD)
    ...
    if pangkalan_induk:
        for date_str, info in pangkalan_induk.items():
            if isinstance(info, dict) and ("haid_hari_ini" in info or "haid_data" in info):
                # Tandakan arkib (archive flag) supaya tidak dikira dalam rumusan aktif semasa
...
with col2:
    ...
    # Proses Carian Data Mengikut Bulan Terpilih
