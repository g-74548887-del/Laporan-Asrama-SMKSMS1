...
# Memastikan data kekal di dalam memori lokal aplikasi sepanjang sesi run
...
with col1:
    ...
    with st.container(height=500):
        for d in all_dates:
            ...
            # Tarikh menjadi hijau jika rekod wujud di dalam session state memori
...
def papar_borang(date_key):
    ...
    if st.button("➕ Tambah Aduan", key="btn_add_aduan"):
        if aduan_text.strip() != "":
            ...
            # Kemas kini terus ke session state lokal dahulu sebelum simpan ke awan
    ...
    if st.button("💾 Simpan Semua Laporan", use_container_width=True, key="btn_save_laporan"):
        # Menyambung aduan dan data haid sedia ada agar tidak terpadam semula menjadi kosong
