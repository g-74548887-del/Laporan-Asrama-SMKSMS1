...
def papar_borang(date_key):
    ...
    with st.container(height=300):
        for m in MURID:
            ...
            col_nama, col_btn1, col_btn2 = st.columns([4, 0.5, 0.5])
            with col_nama:
    ...
    # Ambil semula data imej lama jika ada
    img_data_list = existing.get("program_images", ["", ""])
    
    # Memastikan list sentiasa ada sekurang-kurangnya 2 indeks
    ...
    # Papar semula gambar kecil jika sudah dimuat naik/wujud
...
with col2:
    ...
    # ================== DELETE LAPORAN (PENAMBAHBAIKAN BARU) ==================
    st.subheader("🗑️ Padam Laporan Tarikh")
    
    # Hanya tapis tarikh yang mempunyai laporan yang telah dihantar (berwarna hijau)
    ...
    if senarai_laporan_wujud:
        ...
        if st.button("🔥 Padam Laporan Ini", use_container_width=True, type="primary", key="btn_del_laporan"):
            # Buang tarikh tersebut dari session_state
            st.session_state.data.pop(tarikh_nak_delete, None)
            
            # Jika tarikh yang dipadam sedang aktif dipapar, tutup borang
