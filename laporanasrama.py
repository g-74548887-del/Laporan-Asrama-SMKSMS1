...
def hantar_data_ke_github(data_baru, msg):
    try:
        ...
        st.toast("💾 Data berjaya disimpan ke GitHub!", icon="✅")
    except Exception as e:
...
# ================== LOGIK KIRAAN HAID AKTIF & BULANAN ==================
def kira_rumusan_haid_aktif():
    total_haid = {}
    if st.session_state.data:
        for date_str, info in st.session_state.data.items():
            if isinstance(info, dict):
                if not info.get("reset_haid", False):
...
def analisis_haid_bulanan(bulan_pilihan):
    laporan_bulan = {}
    # Tukarkan integer bulan kepada string format 2 digit (contoh: 6 -> "06")
    str_bulan = f"-{int(bulan_pilihan):02d}-"
    
    if st.session_state.data:
        for date_str, info in st.session_state.data.items():
            # Semak adakah tarikh mengandungi string bulan pilihan (contoh: "2026-06-01")
            if str_bulan in date_str and isinstance(info, dict):
                haid_list = info.get("haid_hari_ini", info.get("haid_data", {}))
                if isinstance(haid_list, dict):
                    for m_nama, v in haid_list.items():
                        if v > 0:
                            if m_nama not in laporan_bulan:
                                laporan_bulan[m_nama] = {"hari": 0, "tarikh": []}
                            laporan_bulan[m_nama]["hari"] += 1
                            laporan_bulan[m_nama]["tarikh"].append(date_str)
    return laporan_bulan
...
# ================== FUNGSI RESET HAID BULAN BARU ==================
def jalankan_reset_haid_bulan_baru():
    pangkalan_induk = ambil_data_dari_github()
    if pangkalan_induk:
        for date_str, info in pangkalan_induk.items():
            if isinstance(info, dict) and ("haid_hari_ini" in info or "haid_data" in info):
                info["reset_haid"] = True
        hantar_data_ke_github(pangkalan_induk, "Tindakan: Reset Haid Bulan Baru dikunci.")
        st.session_state.active_date = None
        st.rerun()
                ...
                for idx in range(start_idx, end_idx):
                    ...
                    if st.button(f"{color} {key}", key=f"list_btn_{key}", use_container_width=True):
                        for k in list(st.session_state.keys()):
                            if k.startswith("temp_images_") and k != f"temp_images_{key}":
...
def papar_popup_laporan(date_key):
    ...
    img_state_key = f"temp_images_{date_key}"
    if img_state_key not in st.session_state:
        st.session_state[img_state_key] = existing.get("images_base64", [])
    ...
    uploaded_files = st.file_uploader("Pilih gambar laporan asrama", type=["png", "jpg", "jpeg"], accept_multiple_files=True, key=f"uploader_{date_key}")
    
    if uploaded_files:
        st.session_state[img_state_key] = []
        for f in uploaded_files[:2]:
            b64_str = base64.b64encode(f.read()).decode("utf-8")
            st.session_state[img_state_key].append(b64_str)

    if st.session_state[img_state_key]:
        cols_img = st.columns(len(st.session_state[img_state_key]))
        for idx, b64_img in enumerate(st.session_state[img_state_key]):
            if b64_img:
    ...
    if st.button("💾 Simpan Semua Rekod Laporan", use_container_width=True):
        existing.update({
            ...
            "images_base64": st.session_state[img_state_key]
        })
        ...
        if img_state_key in st.session_state:
            del st.session_state[img_state_key]
        st.session_state.active_date = None
        st.rerun()
...
with col2:
    ...
    # 1. PAPARAN KITARAN AKTIF SEMASA
    st.write("**🩸 Bilangan Hari Haid (Kitaran Aktif Semasa):**")
    res_haid = kira_rumusan_haid_aktif()
    ada_haid = False
    for nama, bil in sorted(res_haid.items(), key=lambda x: x[1], reverse=True):
        if bil > 0:
            ada_haid = True
            if bil > 8:
                st.markdown(f"<span style='color:red; font-weight:bold;'>👩‍🦰 {nama} ({bil} Hari) 🛑</span>", unsafe_allow_html=True)
            else:
                st.write(f"👩‍🦰 {nama} ({bil} Hari)")
    if not ada_haid:
        st.info("Tiada rekod aktif untuk kitaran bulan semasa.")

    # 2. SEKSYEN ACTION: RESET HAID BULAN BARU
    st.write("")
    sahkan_reset = st.checkbox("Sahkan untuk Reset Haid Bulan Baru?")
    if sahkan_reset:
        if st.button("🚨 KUNCI & RESET HAID BULAN BARU", type="primary", use_container_width=True):
            jalankan_reset_haid_bulan_baru()
            
    st.markdown("---")

    # 3. KAWASAN CARIAN / PENAPIS MURID HAID > 8 HARI MENGIKUT BULAN
    st.markdown("🔍 **Semakan Kesihatan: Haid Melebihi 8 Hari**")
    pilih_bulan = st.selectbox(
        "Pilih Bulan Semakan:", 
        options=[6, 7, 8, 9, 10, 11, 12], 
        format_func=lambda x: f"Bulan {x} (Jun)" if x==6 else f"Bulan {x} (Julai)" if x==7 else f"Bulan {x} (Ogos)" if x==8 else f"Bulan {x} (September)" if x==9 else f"Bulan {x} (Oktober)" if x==10 else f"Bulan {x} (November)" if x==11 else f"Bulan {x} (Disember)"
    )
    
    # Memproses carian data secara terus mengikut perkataan bulan dalam kunci tarikh harian
    data_bulanan = analisis_haid_bulanan(pilih_bulan)
    murid_amaran = {k: v for k, v in data_bulanan.items() if v["hari"] > 8}
    
    if murid_amaran:
        st.warning(f"⚠️ Dikesan {len(murid_amaran)} murid haid > 8 hari pada Bulan {pilih_bulan}:")
        for m_nama, m_info in murid_amaran.items():
            with st.expander(f"🔴 {m_nama} — ({m_info['hari']} Hari)"):
                st.markdown(f"<span style='color:red; font-weight:bold; font-size:16px;'>Nama: {m_nama} ({m_info['hari']} Hari)</span>", unsafe_allow_html=True)
                st.write("**Tarikh direkodkan:**")
                for t in sorted(m_info["tarikh"]):
                    st.write(f"📅 {t}")
    else:
        st.success(f"✅ Tiada murid dikesan haid melebihi 8 hari pada Bulan {pilih_bulan}.")

    st.markdown("---")

    # 4. RUMUSAN SALAH LAKU KESELURUHAN
