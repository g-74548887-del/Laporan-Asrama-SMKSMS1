...
if st.session_state.active_date:
    ...
    def form():
        ...
        kes = st.text_area("Laporan Rondaan", value=existing.get("kes", ""))
        program = st.text_area("Nama Program", value=existing.get("catatan_program", ""))
...
if st.session_state.active_siska:
    ...
    def siska_form():
        ...
        # ===== PAPAR GAMBAR LAMA =====
        if existing.get("gambar1"):
            st.image(base64.b64decode(existing["gambar1"]), caption="Gambar 1")

        if existing.get("gambar2"):
            st.image(base64.b64decode(existing["gambar2"]), caption="Gambar 2")

        # ===== UPLOAD BARU =====
        gambar1 = st.file_uploader("Upload Gambar 1", type=["jpg", "jpeg", "png"])
        gambar2 = st.file_uploader("Upload Gambar 2", type=["jpg", "jpeg", "png"])

        if st.button("Hantar"):

            img1 = img_to_base64(gambar1) if gambar1 else existing.get("gambar1")
            img2 = img_to_base64(gambar2) if gambar2 else existing.get("gambar2")
...
if st.sidebar.button("🧨 Reset Semua Data"):
    ...
    save_json(DENDA_FILE, [], "RESET denda")
    save_json(SISKA_FILE, {}, "RESET siska")
