...
# ================== JANA TARIKH (2026-06-08 HINGGA 2026-12-05) ==================
def gen_dates():
...
pilihan_bulan = st.sidebar.selectbox("Pilih Bulan Paparan Tarikh", ["Jun", "Julai", "Ogos", "September", "Oktober", "November", "Disember"], index=0)
bulan_map = {"Jun": 6, "Julai": 7, "Ogos": 8, "September": 9, "Oktober": 10, "November": 11, "Disember": 12}
bulan_angka = bulan_map[pilihan_bulan]

# ================== LOGIK PENGIRAAN RUMUSAN GLOBAL (SEMUA DATA) ==================
...
def kira_rumusan_salah_laku_keseluruhan():
    ...
    for date_str, info in st.session_state.data.items():
        if isinstance(info, dict):
            # Menggabungkan data aduan lama & salah laku baru supaya kiraan tepat
    ...
    with st.container(height=500):
        for d in all_dates:
            if d.month == bulan_angka:
                ...
                # DIBAIKI DI SINI: Asalkan tarikh ini ada rekod di dalam JSON, ia akan terus tukar HIJAU
...
def papar_popup_laporan(date_key):
    ...
    if salah_laku_list:
        for idx, item in enumerate(salah_laku_list):
            # Sokong paparan bagi format kesalahan baru atau aduan lama
