...
def load_json(path, default):
    try:
        ...
        # Migrasi struktur data lama (haid_data -> haid_hari_ini)
...
def papar_borang(date_key):
    ...
    # ================== KES SALAH LAKU ==================
    st.subheader("📌 Kes Salah Laku")
    aduan_list = existing.get("aduan_list", []) # Kekal key asal json supaya data lama tidak hilang
    murid_pilih = st.selectbox("Nama Murid", MURID, key="sel_murid_salah_laku")
    kesalahan_text = st.text_area("Kesalahan / Isu Salah Laku", key="txt_kesalahan")

    if st.button("➕ Tambah Kes Salah Laku", key="btn_add_kesalahan"):
        if kesalahan_text.strip() != "":
            aduan_list.append({"nama": murid_pilih, "aduan": kesalahan_text})
            existing["aduan_list"] = aduan_list
            st.session_state.data[date_key] = existing
            save_json(FILE_PATH, st.session_state.data, f"Tambah kes salah laku {murid_pilih}")
            st.rerun()
        else:
            st.warning("Sila taip jenis kesalahan terlebih dahulu.")

    st.markdown("### 📌 Rumusan Kes Salah Laku Harian")
    kiraan = {}
    for a in aduan_list:
        kiraan[a["nama"]] = kiraan.get(a["nama"], 0) + 1

    if kiraan:
        for nama_murid, jumlah_kes in kiraan.items():
            colA, colB = st.columns([6, 1])
            with colA:
                st.write(f"{nama_murid} ({jumlah_kes} Kes)")
            with colB:
                if st.button("❌", key=f"del_{nama_murid}_{date_key}"):
                    aduan_list = [a for a in aduan_list if a["nama"] != nama_murid]
                    existing["aduan_list"] = aduan_list
                    st.session_state.data[date_key] = existing
                    save_json(FILE_PATH, st.session_state.data, f"Padam kes {nama_murid}")
                    st.rerun()
    else:
        st.info("Tiada rekod kes salah laku untuk tarikh ini.")
...
with col2:
    ...
    # ================== RUMUSAN KES SALAH LAKU KESELURAHAN ==================
    st.subheader("📌 Rumusan Kes Salah Laku")
    total_aduan = {}
        ...
        for n, v in sorted(total_aduan.items(), key=lambda x: x[1], reverse=True):
            ...
            with colB:
                if st.button("❌", key=f"clear_{n}"):
                    ...
                    save_json(FILE_PATH, st.session_state.data, "clear kes salah laku")
                    st.rerun()
    else:
        st.info("Tiada rekod kes salah laku buat masa ini.")
