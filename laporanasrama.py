...
def get_latest_haid():
    ...
    if st.session_state.data:
        ...
        latest_date = sorted(st.session_state.data.keys())[-1]

        latest_haid = st.session_state.data[latest_date].get("haid_data", {})

        for m in MURID:
            total[m] = latest_haid.get(m, 0)
            ...
            if st.button("❌", key=f"clear_{n}"):
                ...
                for t in st.session_state.data:
                    ...
                    st.session_state.data[t]["aduan_list"] = [
                        a
                        for a in st.session_state.data[t].get("aduan_list", [])
                        if a["nama"] != n
                    ]
