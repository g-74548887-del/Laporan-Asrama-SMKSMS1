...
def analisis_haid_bulanan(bulan_pilihan):
    ...
    if st.session_state.data:
        for date_str, info in st.session_state.data.items():
            try:
                # Menukar string tarikh kepada objek datetime untuk penapisan bulan yang lebih tepat
                tarikh_obj = datetime.datetime.strptime(date_str, "%Y-%m-%d").date()
                if tarikh_obj.month == bulan_pilihan:
                    haid_list = info.get("haid_hari_ini", info.get("haid_data", {}))
                    if isinstance(haid_list, dict):
                        for m, v in haid_list.items():
                            if v > 0:
                                if m not in laporan_bulan:
                                    laporan_bulan[m] = {"hari": 0, "tarikh": []}
                                laporan_bulan[m]["hari"] += 1
                                laporan_bulan[m]["tarikh"].append(date_str)
            except:
                # Cuba alternatif format lain sekiranya ada tarikh yang tidak mengikut format standard YYYY-MM-DD
                try:
                    parts = date_str.split("-")
                    if int(parts[1]) == bulan_pilihan:
                        haid_list = info.get("haid_hari_ini", info.get("haid_data", {}))
                        if isinstance(haid_list, dict):
                            for m, v in haid_list.items():
                                if v > 0:
...
with col2:
    ...
    for nama, bil in sorted(res_haid.items(), key=lambda x: x[1], reverse=True):
        if bil > 0:
            ada_haid = True
            # TUKAR JADI MERAH JIKA LEBIH HARIAN > 8 HARI
            if bil > 8:
    ...
    if murid_amaran:
        ...
        for m_nama, m_info in murid_amaran.items():
            # PAPAR NAMA MERAH DI DALAM EXPANDER CARIAN BULANAN
