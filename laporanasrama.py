...
def get_latest_haid():
    ...
    if st.session_state.data:
        ...
        for t in sorted(st.session_state.data.keys(), reverse=True):
            if st.session_state.data[t].get("haid_data"):
                latest_haid = st.session_state.data[t]["haid_data"]
                break
        ...
        with colH2:
            if st.button("🧨 Reset Laporan (Padam)"):
                if date_key in st.session_state.data:
                    ...
                    save_json(FILE_PATH, st.session_state.data, "delete laporan")
