...
# ================== FUNGSI BACA GITHUB (FIXED: SOKONG FAIL BESAR > 1MB) ==================
def ambil_data_dari_github():
    try:
        # 1. Ambil metadata fail untuk mendapatkan SHA fail terkini
        file_content = repo.get_contents(FILE_PATH)
        
        if file_content.size <= 2:
            return {}
            
        # 2. Guna Git Blob API untuk bypass had saiz get_contents() standard GitHub API
        blob = repo.get_git_blob(file_content.sha)
        raw_content = blob.content
        
        # Decode kandungan Base64 kepada string UTF-8
